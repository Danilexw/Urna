 ...
 */
public class RelatorioController { // [cite: 26]

    @FXML private TextArea relatorioTextArea; // [cite: 18]
    @FXML private Label labelTituloRelatorio; // [cite: 18]
    ...
    public void initialize() { // [cite: 36]
        ...
        gerarRelatorio(); // Gera e exibe o relatório ao inicializar o controlador [cite: 16, 18]
    }

    /**
     * Gera o relatório final da eleição, contabilizando votos e porcentagens. [cite: 16, 18]
     */
    private void gerarRelatorio() { // [cite: 36]
        ...
        Map<String, Long> votosPorCandidato = new HashMap<>(); // [cite: 26]
        List<Candidato> todosCandidatos = candidatoDAO.buscarTodosCandidatos(); // [cite: 26]

        // Contabiliza votos válidos para cada candidato
        for (Candidato candidato : todosCandidatos) { // [cite: 26]
                ...
                .forEach(entry -> {
                    ...
                    if (candidato != null) {
                        ...
                        relatorio.append(String.format("Candidato %s (%s): %.0f%% (%d votos)\n",
                                candidato.getNome(), candidato.getId(), porcentagem, votos)); // [cite: 18]
                    }
                });
